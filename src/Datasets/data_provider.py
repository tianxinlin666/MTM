...
def collate_train(data, cfg):
    ...
    target = torch.zeros(len(all_lengths), max_desc_l, feat_dim)
    words_mask = torch.zeros(len(all_lengths), max_desc_l)

    for index, cap in enumerate(merge_captions):
        end = min(all_lengths[index], max_desc_l)
...
def collate_frame_val(data, cfg):
    ...
    return clip_videos, clip_videos_mask, frame_videos, videos_mask, idxs, video_ids

def collate_text_val(data, cfg):
